...
export class MealPlanningConversationChain {
  ...
  private sessions: Map<string, MealPlanningSession> = new Map()
  ...
  async acceptRecipe(sessionId: string, recipeId: number): Promise<string> {
    ...
    session.declinedRecipes = session.declinedRecipes.filter(id => id !== recipeId)
  ...
  async declineRecipe(sessionId: string, recipeId: number, reason?: string): Promise<string> {
    ...
    session.acceptedRecipes = session.acceptedRecipes.filter(id => id !== recipeId)
